...
from mainapp.models import Categories, SubCategories, CustomUser, MerchantUser, Products, ProductAbout, \
    ProductDetails, ProductMedia, ProductTransaction, ProductTags, StaffUser, CustomerUser
...
from ecommerce.settings import BASE_URL
...
class SubCategoriesListView(ListView):
    ...
    def get_queryset(self):
        ...
        order_by = self.request.GET.get("orderby", "id")
        if filter_val != "":
            cat = SubCategories.objects.filter(
                Q(title__contains=filter_val) | Q(description__contains=filter_val)).order_by(order_by)
        else:
...
class MerchantUserListView(ListView):
    ...
    def get_queryset(self):
        ...
        order_by = self.request.GET.get("orderby", "id")
        if filter_val != "":
            cat = MerchantUser.objects.filter(
                Q(auth_user_id__first_name__contains=filter_val) | Q(auth_user_id__last_name__contains=filter_val) | Q(
                    auth_user_id__email__contains=filter_val) | Q(
                    auth_user_id__username__contains=filter_val)).order_by(order_by)
        else:
    ...
    def get_queryset(self):
        ...
        if filter_val != "":
            products = Products.objects.filter(
                Q(product_name__contains=filter_val) | Q(product_description__contains=filter_val)).order_by(order_by)
        else:
...
class ProductEdit(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, "admin_templates/product_edit.html",
                      {"categories": categories_list, "product": product, "product_details": product_details,
...
class ProductAddStocks(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        product_transaction = ProductTransaction(product_id=product_obj, transaction_product_count=new_instock,
                                                 transaction_description="New Product Added", transaction_type=1)
        product_transaction.save()
...
class CustomerUserListView(ListView):
    ...
    def get_queryset(self):
        ...
        order_by = self.request.GET.get("orderby", "id")
        if filter_val != "":
            cat = CustomerUser.objects.filter(
                Q(auth_user_id__first_name__contains=filter_val) | Q(auth_user_id__last_name__contains=filter_val) | Q(
                    auth_user_id__email__contains=filter_val) | Q(
                    auth_user_id__username__contains=filter_val)).order_by(order_by)
        else:
